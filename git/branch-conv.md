...
1. **Перед релизом должен быть сделан дамп базы данных и сделан снапшот(бекап) у хостинг провайдера.**
...
6. **После обновления тестировщик должен проверить все загруженные задачи на продакшн сервере.**
- Примеры:
```
release/1.01.123
```
