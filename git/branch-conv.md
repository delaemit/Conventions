...
Продакшн-ветка, если репозиторий опубликован, это основная ветка, которая отображается по умолчанию.
С этой ветки разработчики создают временные `feature` ветки под каждую задачу.
